  ...
  Movie
    ...
    .then(movies => {
      return Director// tutto questo per mostrare i film del regista selezionato
        ...
        .exec()
        .then(directors => {
          res.render('movies/index', { movies, directors, selectedDirector: req.query.director, activeM: true });
        });
    })
...
function moviesShow(req, res) {
  Movie
    ...
    .then((movie) => {
      ...
      res.render('movies/show', {movie, activeM: true});
    })
    .catch((err) => {
...
function moviesNew(req, res) {
  Director
    ...
    .exec()
    .then((directors) =>{
      res.render('movies/new', {directors, activeNm: true});
    })
    .catch((err) => {
      res.status(500).render('error', { err });
    });
